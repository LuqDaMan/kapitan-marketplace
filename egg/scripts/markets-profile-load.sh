...
PROFILE="$HOME/.claude/markets-teacher-profile.md"
LEDGER="$HOME/.claude/markets-teacher-ledger.md"
...
<!-- Write your goals, target firms, asset class focus, preferred level, and timeline here. Claude reads this but won't overwrite without asking. -->
...
LEDGER_TEMPLATE
  ...
  echo "=== MARKETS PROFILE (new — first session) ==="
  cat "$PROFILE"
  echo ""
...
if [ -n "$LAST_PROFILE_TS" ] && [ -n "$LAST_LEDGER_TS" ]; then
  if [[ "$LAST_PROFILE_TS" > "$LAST_LEDGER_TS" ]]; then
    ...
    echo "| ${LAST_PROFILE_TS} | sync-heal | ${P_TOPIC} | ${P_ASSET} | ${P_MODE} | ${P_VERDICT} | ${P_GAPS} | ${P_REVIEW} |" >> "$LEDGER"
